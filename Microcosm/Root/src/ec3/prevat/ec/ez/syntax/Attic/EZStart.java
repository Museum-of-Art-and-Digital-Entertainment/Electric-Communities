...
public class EZStart {
         ...
         InstantiationException, IOException, OnceOnlyException
    {
        ...
        ELaunchable_$_Intf starter;
/* Turned off
        if(args.length >= 1) {
        Object obj = Class.forName(args[0] + "_$_Impl").newInstance();

            try {
                starter = (ELaunchable_$_Intf)obj;
...
*/
        ...
        new TraceController(env.props());

        vat.setEEnvironment(env);
        starter.go$async(env);
...
class ezdummy implements ELaunchable {

    ezdummy() {}

    public void go$async(EEnvironment env) {
...
// If requested, fire up the registrar
   ...
   //           registrar.onTheAir(env.getProperty("SearchPath"));
    ...
    public void order$async(RtEnvelope dum1, EDistributor_$_Intf dum2) {

    }

    public void when$async(EWhenClosure_$_Intf dummy) {
