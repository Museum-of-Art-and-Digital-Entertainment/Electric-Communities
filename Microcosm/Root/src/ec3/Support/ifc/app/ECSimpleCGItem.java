...
/**
 * This is the most general ECControlGroupItem subclass. It combines
 ...
 * are determined from the height of the ECSimpleCGItem and
 * the adjustable contentsMargin(). <P>
 * You can subclass from ECSimpleCGItem to add additional subviews.
 */
public class ECSimpleCGItem extends ECControlGroupItem {
    ...
    public TextField getLabelField() {
        if (myLabelField == null) {
            setLabelField(TextField.createLabel(""));
        }
        return myLabelField;
    }
    ...
    /**
     * Sets the control's fixed width, but does not reposition the contents.
     * To reposition the contents, call layoutContents or relayoutContents.
    ...
    /**
     * Sets the control horizontal margin, but does not reposition the
     * control. To reposition control, call layoutContents or
     * relayoutContents.
    ...
    /**
     * Sets whether the control resizes automatically, but does not
     * reposition the contents. To reposition the contents, calle
     * layoutContents or relayoutContents.
     * @see #controlResizesAutomatically
     */
    ...
    /**
     * Sets the control vertical margin, but does not reposition the
     * control. To reposition control, call layoutContents or
     * relayoutContents.
     * Values less than zero are converted to zero.
     ...
     * If the label has embedded "\n"s, it will be split onto multiple lines.
     */
    ...
    public void setLabelField(TextField newField) {
        ...
        if (myLabelField != null) {
            ...
            // label field cannot be editable, because wrapsContents would
            // not work if it were
            myLabelField.setEditable(false);
            // by letting it wrap contents, we can have multi-line labels
            myLabelField.setWrapsContents(true);
