...
// gcc on Linux defines unix, but not UNIX
#ifdef unix
#define UNIX 1
#endif
...
JNIEXPORT jlong JNICALL package(Native_queryPerformanceCounter)(
    JAVA_PARAMS(Native))
{
#ifdef UNIX
    struct timeval  tp;
    ...
    QueryPerformanceCounter(&li);
    return (jlong)li.HighPart*((jlong)ULONG_MAX+1) + (jlong)li.LowPart;
#endif
}
