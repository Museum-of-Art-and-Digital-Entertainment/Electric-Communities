...
public class InspectorCrew {

    /**

     * gather() (in its six variants - three here and three in
     * guest.inspector) will gather the 'object' argument into a
    ...
    public static void gather(Object iobj, String category, String name) {
        ...
        synchronized(ec.e.inspect.Inspector.getGatheredCrewObjects()) { // Currently inspected crew objects

            // See if the object already exists in the global object collection

            Object oldName = ec.e.inspect.Inspector.getGatheredCrewObjects().get(iobj);
            ...
            if (category == null) catTable = ec.e.inspect.Inspector.getCrewObjectsWithoutCategory();
            else {
                synchronized(ec.e.inspect.Inspector.getCrewObjectCategories()) {
                    catTable = (Hashtable)ec.e.inspect.Inspector.getCrewObjectCategories().
                      get(category);
                    if (catTable == null) { // No entries under this category name before
                        catTable = new Hashtable(10); // so add the category
                        ec.e.inspect.Inspector.getCrewObjectCategories().
