...
public class Decrypt3DES {
    ...
    private static final Trace tr = new Trace("ec.e.net.Decrypt3DES");
    ...
    public Decrypt3DES(byte[] desKeys, byte[] iv) {
        myDESKeys = desKeys;
        myIV = iv;
    ...
    public void decrypt(byte[] buffer, int off, int len) {
        ...
        if (tr.debug && Trace.ON) tr.debugm("Pkt3DESDecrypt(" + buffer.length + "), time " 
