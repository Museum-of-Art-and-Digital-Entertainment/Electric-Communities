...
eCompile:
        '.' compilationUnit
{
    YC(compilationUnit,$2)->finalWS = flushWhitespace();
    YRESULT($2);
}
 ;
        ...
        PACKAGE name ';'
{
    ...
    CurrentUnitInfo = newUnitInfo(YC(name,$2));
}
 |
{
        ...
        IMPORT name ';'
{
    ...
    performImportClass(YC(name,$2), CurrentUnitInfo);
}
 |      IMPORT name '.' '*' ';'
{
    $$ = YH_BUILD(importDeclaration)(WS($1), YC(name,$2), WS($3), WS($4),
                                     WS($5));
    performImportPackage(YC(name,$2), CurrentUnitInfo->importedClasses);
}
 ;
...
{
    YT(class) *theClass = 
        defineClass(CurrentUnitInfo, YC(identifier,$3), FALSE, FALSE);
    $$ = YH_BUILD(classDeclaration)(YC(modifierList,$1), WS($2),
        YC(identifier,$3), YC(extends,$4), YC(implements,$5),
        YC(classBody,$6), theClass);
    attachDeclaration(theClass, YC(typeDeclaration,$$));
}
 ;
...
{
    YT(class) *theClass =
        defineClass(CurrentUnitInfo, YC(identifier,$3), TRUE, FALSE);
...
{
    YT(class) *theClass =
        defineClass(CurrentUnitInfo, YC(identifier,$3), FALSE, TRUE);
...
{
    YT(class) *theClass =
        defineClass(CurrentUnitInfo, YC(identifier,$3), TRUE, TRUE);
...
block:
        '{' blockStatements '}'
{
    $$ = YH_BUILD(block)(WS($1), YC(statementList,$2), WS($3));
}
 |      '{'                 '}'
{
    $$ = YH_BUILD(block)(WS($1), NULL, WS($2));
}
 ;
