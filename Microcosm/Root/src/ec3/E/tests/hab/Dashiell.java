...
/**
  ...
  Haberdashery.
*/
public class Dashiell {
    static Trace t = new Trace("ec.tests.hab.Dashiell"); /* XXX debug only */

    private DataInputStream in;             /* command input */
    ...
    /**
    * Launch a new Dashiell
    */
    public static void main(String args[]) {
        (new Dashiell(System.in, System.out)).processCommands();
    }

    /**
    * Construct a new shell. You specify the input and output streams so that
    * maybe someday this won't need to use the console.
    */
    private Dashiell(InputStream in, PrintStream out) {
    ...
    */
    ...
    /**
    * Read commands and process them until one of the commands says to quit or
    * we stumble over an unrecoverable error.
    */
    private void processCommands() {
        ...
        while (!done) {
            try {
                prompt();
                processNextCommand(in.readLine());
            } catch (IOException e) {
                done = true;
            } catch (DashiellFatalErrorException e) {
                done = true;
            } catch (DashiellNonFatalErrorException e) {
                done = false;
            } catch (Exception e) {
                out.println("serious internal error: " + e.getMessage());
    ...
    */
    private void processNextCommand(String line) {
    ...
    */
    private DirectDesignator doAddMapping(String name, DirectDesignator des,
            Unit unit, BasedDesignator unitDes, boolean purgeFlag) {
        Unit newUnit;
        if (des == null)
