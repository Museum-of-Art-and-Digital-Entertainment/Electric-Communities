...
/* 
   ...
   java ec.e.start.EBoot ec.tests.rep.DeletionTest

   to a Windows bash shell.
   You can remove all inspector code if you want.
   */
 

eclass DeletionTest implements ELaunchable {
    emethod go(EEnvironment env) {
        DeletionTester tester = new DeletionTester(env);
        tester <- go();
    }
}

eclass DeletionTester {
    ...
    public DeletionTester(EEnvironment env) {
        myEnv = env;
    }
    ...
    emethod go() {
        ...
        //        ECApplication app = new ECApplication(); // Create the IFC app
        ...
        try {
            ...
            Repository rep = new Repository(anEditor,true); // Writable - add the "true"
            ...
            EFileEditor anotherEditor = anEditFile.editor();
            Repository sameRep = new Repository(anotherEditor,true); // Writable - add the "true"

            // ec.e.inspect.Inspector.inspect(null,null); // Force inspector window to appear

            Canine deletedDog = (Canine)sameRep.get("dog4");
            if (deletedDog != null) EStdio.err().println("[FAILURE] DOGNDEL - Delete failure");
            else EStdio.err().println("[SUCCESS] DOGGONE - Dog successfully deleted");
            sameRep.close();
            // app.run();
