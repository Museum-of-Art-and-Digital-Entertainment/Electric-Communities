...
import ec.e.rep.steward.*;      // Needed to use Repository
import ec.tests.rep.Feline; // Needed only for Feline class used in these examples

/* You may want to define
alias rtst 'java -debug ec.e.start.EBoot ec.e.start.Agency Agent=ec.tests.rep.rtst'
*/

eclass altst implements ELaunchable
{
  emethod go (EEnvironment env) {   
    try {
      ...
      RepositoryAlias ra = new RepositoryAlias("self:cat1",RepositoryAlias.RA_RETRIEVE); // Don't cache the results
      if (ra == null)  System.out.println("[FAILURE] ALLESS - Could not create alias");

      repry.put("cat1",cat1);   // Store under name cat1
      repry.put("cat-alias",ra); // Store alias to cat1 in same repository
      ...
      Feline cat2 = (Feline)repry1.get("cat-alias");    // Get it back from Repository
